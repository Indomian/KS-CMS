...
<ul class="nav" id="navChain">
    ...
    <li><a href="{get_url _CLEAR="ACTION type CSC_catid id i p1 CSC_id"}"><img src="{#images_path#}/icons_menu/arrow.gif" alt="icon_arrow" height="13" width="13" />&nbsp;<span>{#title_releases#}</span></a></li>
...
<div class="users">
    ...
    <tr>
    	...
    	<th width="25%">
    		<a href="{get_url _CLEAR="PAGE" order=theme dir=$order.newdir}">{#theme#}</a>{if $order.field=='theme'}&nbsp;<img src="{#images_path#}/{if $order.curdir=='asc'}up{else}down{/if}.gif">{/if}
    	</th>
    	<th width="15%">
    		<a href="{get_url _CLEAR="PAGE" order=from dir=$order.newdir}">{#release_from#}</a>{if $order.field=='from'}&nbsp;<img src="{#images_path#}/{if $order.curdir=='asc'}up{else}down{/if}.gif">{/if}
    	</th>
    	<th width="20%">
    		{#newsletter#}
    	</th>
    	<th width="15%">
    		<a href="{get_url _CLEAR="PAGE" order=send dir=$order.newdir}">{#state#}</a>{if $order.field=='send'}&nbsp;<img src="{#images_path#}/{if $order.curdir=='asc'}up{else}down{/if}.gif">{/if}
    	</th>
    	<th width="10%">
    		<a href="{get_url _CLEAR="PAGE" order=date_add dir=$order.newdir}">{#date_create#}</a>{if $order.field=='date_add'}&nbsp;<img src="{#images_path#}/{if $order.curdir=='asc'}up{else}down{/if}.gif">{/if}
    	</th>
...
{strip}
<dl class="def" style="background:#FFF6C4 url('{#images_path#}/big_icons/feedback.gif') left 50% no-repeat;{if $smarty.cookies.showHelpBar==1}display:none;{/if}">
	<dt>Управление выпусками рассылок</dt>
	<dd>Данный раздел модуля позволяtет создавать новые выпуски рассылки в ручном режиме, а также управлять уже созданными.</dd>        
</dl> 
