...
<div class="users">
    ...
    <tr>
    	...
    	<th width="20%">
    		<a href="{get_url _CLEAR="PAGE" order=name dir=$order.newdir}">{#name#}</a>{if $order.field=='name'}&nbsp;<img src="{#images_path#}/{if $order.curdir=='asc'}up{else}down{/if}.gif">{/if}
    	</th>
    	<th width="40%">
    		<a href="{get_url _CLEAR="PAGE" order=description dir=$order.newdir}">{#description#}</a>{if $order.field=='description'}&nbsp;<img src="{#images_path#}/{if $order.curdir=='asc'}up{else}down{/if}.gif">{/if}
    	</th>
    	<th width="10%">
    	...
    	<th width="5%" style="text-align: center;"><a href="{get_url order=active dir=$order.newdir}">Активность</a>&nbsp;{if $order.field=='active'}<img src="{#images_path#}/{if $order.curdir=='asc'}up{else}down{/if}.gif"/>{/if}</th>
    	<th></th>
    </tr>
