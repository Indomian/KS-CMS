...
{
	$sResult='<table class="layout"><tr><td width="30%">Модуль к разделам которого осуществлять привязку: </td>' .
		'<td width="70%"><select name="'.$params['prefix'].'option_1" id="'.$params['prefix'].'option_1">';
	foreach($arModules as $arItem)
	{
		$sResult.='<option value="'.$arItem['directory'].'">'.$arItem['name'].'</option>';
	}
	...
	$sResult.='<script type="text/javascript">' ."\n".
			'function showForm'.$rand.'(e,data){'."\n".
				'var obData=$(":checkbox[name=\'sel\[cat\]\[\]\']",data);'."\n".
			...
			'$("input#select").click(function(event){'."\n".
				'if($("#'.$params['prefix'].'option_1").val()=="blog")'.
					'kstb_show("Выбрать раздел","/admin.php?module="+$("#'.$params['prefix'].'option_1").val()+"&page=sections&mode=small&width=800&height=480",null,showForm'.$rand.');'.
				'else kstb_show("Выбрать раздел","/admin.php?module="+$("#'.$params['prefix'].'option_1").val()+"&mode=small&width=800&height=480",null,showForm'.$rand.');'.
			'})});'.
		'</script>';
}
else
