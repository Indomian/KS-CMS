...
class ImageResizer extends CBaseObject
{
	...
	var $image_type;              // Тип изображения
  	...
  	var $border_color = "206 206 206";             // Цвет "белых" полей.
	 ...
	 * */
	function Resize($image_w,$image_h,$image_ratio=true, $image_ratio_wb=true,$newdir=false)
	{
		...
		*/
		...
		{
			$this->createpng($image_w,$image_h,$image_ratio,$image_ratio_wb);
		}
		else
		{
	...
	function createpng($image_w,$image_h,$image_ratio,$image_ratio_wb)
	{
		$im = ImageCreateTrueColor($image_h,$image_w);
		imagesavealpha($im, TRUE);
		$cc=imagecolorallocatealpha($im, 255, 255, 255, 127);
		imagefill($im, 0, 0, $cc);
		$imr = imagecreatefrompng($this->myfile);
		imagealphablending($imr, false);
		imagesavealpha($imr, true);
		if($this->width_orig > $this->height_orig || $this->width_orig == $this->height_orig)
		{
			imagecopyresampled($im, $imr, 0, 0, round((max($this->width_orig,$this->height_orig)-min($this->width_orig,$this->height_orig))/2),
					 	0, $image_w, $image_h,
		...
		imagecopyresampled($im, $imr, 0, 0, 0, $start, $image_w, $image_h,
					              min($this->width_orig,$this->height_orig), min($this->width_orig,$this->height_orig));
		}
		if($this->isSave == true)
	    {
	...
	function Process($image_w,$image_h,$image_ratio,$image_ratio_wb)
	{
	    ...
	    else
	    {
	    	if($image_ratio_wb == true)
	    	{
    			...
    			$background_color = imagecolorallocate($newImg_first, $r, $g, $b);
				imagefill($newImg_first, 0, 0, $background_color);
