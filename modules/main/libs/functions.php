...
/**
 * Функция проверяет является ли строка md5 хэшем
 */
function IsHash($hash)
{
	return preg_match('#^[a-f0-9]{32,32}$#i',$hash);
}
...
/**
 * Функция проверяет является ли строка именем
 */
function IsName($str)
{
	return preg_match('#^[a-zабвгдеёжзийклмнопрстуфхцчшщьъыэюяАБВГДЕЁЖЗИЁКЛМНОПРСТУФХЦЧШЩЬЫЪЭЮЯ\-\ ]+$#iu',$str);
}

/**
 * Функция проверяет является ли строка названием города
 */
function IsCityName($str)
{
	return preg_match('#^[a-z0-9абвгдеёжзийклмнопрстуфхцчшщьъыэюяАБВГДЕЁЖЗИЁКЛМНОПРСТУФХЦЧШЩЬЫЪЭЮЯ\-\ ]+$#iu',$str);
}
