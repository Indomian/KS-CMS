...
{$data.date_add|date_format:"%d.%m.%Y %H:%M"} пользователем {if $data.user.is_author==1}{$data.user.nickname}{else}{if $data.user.name!='' || $data.user.last_name!=''}{$data.user.name} {$data.user.last_name}{else}{$data.user.title}{/if}{/if} был добавлен ответ на ваш комментарий {if $data.is_profile}к профилю{else}к рингтону{/if}.
-------------

{$data.post}

-------------
Для просмотра ответа, пожалуйста, перейдите по ссылке {$SITE.home_url}{$data.url}.
